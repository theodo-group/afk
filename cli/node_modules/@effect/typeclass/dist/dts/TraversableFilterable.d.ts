/**
 * `TraversableFilterable` represents data structures which can be _partitioned_ with effects in some `Applicative` functor.
 *
 * @since 0.24.0
 */
import type { Either } from "effect/Either";
import type { Kind, TypeClass, TypeLambda } from "effect/HKT";
import type { Option } from "effect/Option";
import type { Applicative } from "./Applicative.js";
import type { Covariant } from "./Covariant.js";
import type { Filterable } from "./Filterable.js";
import type { Traversable } from "./Traversable.js";
/**
 * @category models
 * @since 0.24.0
 */
export interface TraversableFilterable<T extends TypeLambda> extends TypeClass<T> {
    readonly traversePartitionMap: <F extends TypeLambda>(F: Applicative<F>) => {
        <A, R, O, E, B, C>(f: (a: A) => Kind<F, R, O, E, Either<C, B>>): <TR, TO, TE>(self: Kind<T, TR, TO, TE, A>) => Kind<F, R, O, E, [Kind<T, TR, TO, TE, B>, Kind<T, TR, TO, TE, C>]>;
        <TR, TO, TE, A, R, O, E, B, C>(self: Kind<T, TR, TO, TE, A>, f: (a: A) => Kind<F, R, O, E, Either<C, B>>): Kind<F, R, O, E, [Kind<T, TR, TO, TE, B>, Kind<T, TR, TO, TE, C>]>;
    };
    readonly traverseFilterMap: <F extends TypeLambda>(F: Applicative<F>) => {
        <A, R, O, E, B>(f: (a: A) => Kind<F, R, O, E, Option<B>>): <TR, TO, TE>(self: Kind<T, TR, TO, TE, A>) => Kind<F, R, O, E, Kind<T, TR, TO, TE, B>>;
        <TR, TO, TE, A, R, O, E, B>(self: Kind<T, TR, TO, TE, A>, f: (a: A) => Kind<F, R, O, E, Option<B>>): Kind<F, R, O, E, Kind<T, TR, TO, TE, B>>;
    };
}
/**
 * Returns a default binary `traversePartitionMap` implementation.
 *
 * @since 0.24.0
 */
export declare const traversePartitionMap: <T extends TypeLambda>(T: Traversable<T> & Covariant<T> & Filterable<T>) => <F extends TypeLambda>(F: Applicative<F>) => <TR, TO, TE, A, R, O, E, B, C>(self: Kind<T, TR, TO, TE, A>, f: (a: A) => Kind<F, R, O, E, Either<C, B>>) => Kind<F, R, O, E, [Kind<T, TR, TO, TE, B>, Kind<T, TR, TO, TE, C>]>;
/**
 * Returns a default binary `traverseFilterMap` implementation.
 *
 * @since 0.24.0
 */
export declare const traverseFilterMap: <T extends TypeLambda>(T: Traversable<T> & Filterable<T>) => <F extends TypeLambda>(F: Applicative<F>) => <TR, TO, TE, A, R, O, E, B>(self: Kind<T, TR, TO, TE, A>, f: (a: A) => Kind<F, R, O, E, Option<B>>) => Kind<F, R, O, E, Kind<T, TR, TO, TE, B>>;
/**
 * @since 0.24.0
 */
export declare const traverseFilter: <T extends TypeLambda>(T: TraversableFilterable<T>) => <F extends TypeLambda>(F: Applicative<F>) => {
    <B_1 extends A, R, O, E, A = B_1>(predicate: (a: A) => Kind<F, R, O, E, boolean>): <TR, TO, TE>(self: Kind<T, TR, TO, TE, B_1>) => Kind<F, R, O, E, Kind<T, TR, TO, TE, B_1>>;
    <TR, TO_1, TE_1, B extends A_1, R_1, O_1, E_1, A_1 = B>(self: Kind<T, TR, TO_1, TE_1, B>, predicate: (a: A_1) => Kind<F, R_1, O_1, E_1, boolean>): Kind<F, R_1, O_1, E_1, Kind<T, TR, TO_1, TE_1, B>>;
};
/**
 * @since 0.24.0
 */
export declare const traversePartition: <T extends TypeLambda>(T: TraversableFilterable<T>) => <F extends TypeLambda>(F: Applicative<F>) => {
    <B_1 extends A, R, O, E, A = B_1>(predicate: (a: A) => Kind<F, R, O, E, boolean>): <TR, TO, TE>(self: Kind<T, TR, TO, TE, B_1>) => Kind<F, R, O, E, [Kind<T, TR, TO, TE, B_1>, Kind<T, TR, TO, TE, B_1>]>;
    <TR, TO_1, TE_1, B extends A_1, R_1, O_1, E_1, A_1 = B>(self: Kind<T, TR, TO_1, TE_1, B>, predicate: (a: A_1) => Kind<F, R_1, O_1, E_1, boolean>): Kind<F, R_1, O_1, E_1, [Kind<T, TR, TO_1, TE_1, B>, Kind<T, TR, TO_1, TE_1, B>]>;
};
//# sourceMappingURL=TraversableFilterable.d.ts.map