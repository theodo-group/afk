...
/** @internal */
export const makeProvider = (fileName, options) => Effect.gen(function* (_) {
  ...
  const filePaths = yield* _(Effect.filter(searchPaths.flatMap(searchPath => extensions.map(ext => path.join(searchPath, `${fileName}.${ext}`))), path => Effect.orElseSucceed(fs.exists(path), () => false)));
  const providers = yield* _(Effect.forEach(filePaths, path => pipe(fs.readFileString(path), Effect.mapError(_ => ConfigFileError(`Could not read file (${path})`)), Effect.flatMap(content => Effect.mapError(InternalFiles.parse(path, content), message => ConfigFileError(message))), Effect.map(data => ConfigProvider.fromJson(data)))));
