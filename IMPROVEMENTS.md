...
**15.14 `afk logs` ignores the active backend ⏳ (CF broken).** On the Cloudflare backend, `afk logs <id>` calls the **AWS** CloudWatch adapter (`FilterLogEvents`, `cli/src/adapters/aws/Logs.ts`) and fails with `ResourceNotFoundException`. The command isn't dispatched through the backend's LogStore — it needs the CF path (`wrangler tail` / Workers Logs) like the other commands. Until fixed, use `wrangler tail afk-launcher`.

**15.15 CF Run status stuck at PROVISIONING ⏳.** After launch the Run stays `PROVISIONING` in `afk ls` — the RegistryDO row is written once (hardcoded `PROVISIONING` in the launcher `/runs` handler) and never transitioned to RUNNING/STOPPED. `RunDO.markRunning()`/`markStopped()` update the per-Run DO but the registry index the `ls` reads from isn't updated, and there's no container-state poll. Needs the DO to push status changes to RegistryDO (and ideally reflect actual Container lifecycle).
