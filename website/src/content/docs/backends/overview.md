...
| Backend | Compute primitive | Provisioning | Capacity | Notes |
| --- | --- | --- | --- | --- |
| **[AWS EC2](/backends/aws/)** | One EC2 VM per Run | Terraform (VPC, IAM, sweeper Lambda, DynamoDB, S3 state) | Spot by default (`--on-demand` opts out) | Full Compose Contract; host Docker daemon |
| **[GCP Compute Engine](/backends/gcp/)** | One Compute Engine VM per Run | Terraform (VPC + NAT + IAP, service accounts, Firestore, Artifact Registry, GCS state) | Spot by default (`--on-demand` opts out) | Full Compose Contract; attach over IAP tunnel |
| **[Cloudflare Containers](/backends/cloudflare/)** | One Container instance per Run | Customer-deployed launcher Worker (rootless `dind`) | n/a | Requires the Workers Paid plan |
| **[Local](/backends/local/)** | One container per Run on your own daemon | None (`afk provision` is a no-op) | n/a | Rootless `dind`; needs only Docker |
...
See the [CLI surface](/reference/cli/) for the complete command reference.
