...
See the [Quickstart](/getting-started/quickstart/#on-aws) for the setup commands. This document covers what the backend provisions and how its attach / lifecycle / cost specifics work.
...
- Spot by default (cheaper, but interruptible — a Spot interruption kills the Run); `--on-demand` opts into pricier capacity AWS won't preempt. Neither is retained: every cloud Run terminates on exit, so `afk attach` only enters a *live* Run (declare a Session Artifact to capture post-Run state).
