---
title: GCP Compute Engine Backend
description: "One Compute Engine VM per Run booted from a custom-image Golden Image, with attach over an IAP tunnel and OS Login."
---
...
- An **`afk-vm`** service account attached to every Run VM. Grants: Artifact Registry reader; Secret Manager `secretAccessor` scoped to `afk-*` secrets via an IAM condition; Logging `logWriter`; Storage `objectCreator` on the artifacts bucket; and a custom self-delete role holding only `compute.instances.delete`. Nothing else.
- An **`afk-developer`** custom role bound to developer principals: `compute.instances.create` (conditioned on the AFK subnet + machine-type whitelist where IAM conditions allow), `compute.instances.delete`, `iam.serviceAccountUser` on `afk-vm` only (the PassRole analogue — a developer can attach only the `afk-vm` SA), `roles/iap.tunnelResourceAccessor`, and `roles/compute.osLogin`.
- An **`afk-sweeper`** SA for the Cloud Function: read-only `compute.instances.list/get` + `roles/datastore.user`. No delete.
...
- **Timeout backstop is native GCE**: every instance is created with `scheduling.max_run_duration` (from `--timeout`, default capped by `max_run_timeout_hours`) and `instance_termination_action=DELETE`. If the agent crashes before the entrypoint reaches self-delete, GCE deletes the VM when the duration elapses — there is **no instance-reaping sweeper** (unlike AWS).
- The **history-reconcile Cloud Function** (Cloud Scheduler, every 5 min) only rewrites orphaned `RUNNING` Firestore rows to `STOPPED` (`stoppedAt=now`, `exitCode=null`) for Runs whose VM has vanished without writing completion. It never deletes VMs.
...
- Per-Run: Compute Engine billed per second at the chosen machine type, boot disk, Cloud Logging ingest, NAT egress. **Spot by default** (`provisioning-model=SPOT`, 60–91% off standard) — `--on-demand` opts up to `STANDARD` for interruption-resistance on long Runs. A Spot reclaim DELETEs the VM mid-Run, same as a clean exit; the reconcile Cloud Function flips the orphaned history row. (Modern Spot VMs support `max_run_duration` + `instance_termination_action=DELETE`; this is not the legacy preemptible product.)
