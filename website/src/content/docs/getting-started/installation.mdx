...
<Tabs>
  <TabItem label="AWS">
    ...
    - **`npm`** (the sweeper Lambda is bundled with esbuild at
      `terraform apply` time)
  </TabItem>
  <TabItem label="GCP">
    - **Terraform ≥ 1.10** (GCS native state locking)
    - **`gcloud` CLI** authenticated (`gcloud auth login`); the active account
      is the Run's Owner
    - A **GCP project** with billing enabled, selected via
      `gcloud config set project <id>`
    - **OS Login + IAP TCP forwarding** (`roles/iap.tunnelResourceAccessor`,
      granted by the module) for `afk attach` — no public IP, no SSH
  </TabItem>
  <TabItem label="Cloudflare">
    ...
    - **`CLOUDFLARE_API_TOKEN`** in a gitignored `.env` at the repo root (afk
      ...
      `Workers KV Storage:Edit`, and `Access: Service Tokens:Edit` (the last
      only for `afk team add`)
    - A Cloudflare account on the **Workers Paid plan** (Containers requires
      it)
  </TabItem>
  <TabItem label="Local">
